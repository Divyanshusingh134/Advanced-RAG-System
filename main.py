...
class GeminiClient:
    ...
    async def generate(self, http_client: httpx.AsyncClient, prompt: str):
        payload = {
        ...
        for attempt in range(self.max_retries):
            try:
                response = await http_client.post(url=self.generation_url,json=payload)
                ...
                if not candidate:
                    ...
                    continue
                logging.info(f"Successfully Generated the Response")
...
class DocumentReranker:
    ...
    async def rerank(self, retrieved_chunks: list[str], query: str, top_k:int = 3) ->list[str]:
    ...
    async def relevance_score(self, result: str, query: str) -> float:
        pair = [[query, result]]
...
class Evaluator:
    def __init__(self, llm_client: GeminiClient, reranker: DocumentReranker):
        self.llm = llm_client
        self.reranker = reranker

    async def get_faithfulness_score(self, http_client: httpx.AsyncClient, retrieved_chunks: list[str], query: str, result: str) -> int:
        ...
        score = await self.llm.generate(http_client=http_client, prompt=prompt)
        try:
            score = int(score)
        except ValueError:
    ...
    async def get_relevance_score(self, query: str, result: str) -> float:
        return await self.reranker.relevance_score(result=result, query=query)
...
class RAGPipeline:
    def __init__(self, api_key: str):
        ...
        self.evaluator = Evaluator(llm_client=self.llm, reranker=self.reranker)
    ...
    async def run_evaluator(self, data_file: str, queries_file: str, output_file: str):
        ...
        async with httpx.AsyncClient(timeout=HTTP_TIMEOUT, headers=self.llm.headers) as http_client:
            ...
            query_vectors = await self.llm.embed(http_client=http_client, chunks=queries)
            with open(output_file, "a+") as file:
                file.write("## RAG Evaluation Results\n\n")
                stats = {
                    "sentence":{"relevance": 0.0, "faithful": 0},
                    "fixed": {"relevance": 0.0, "faithful": 0}
                }
                for k, query in enumerate(queries):
                    if not query_vectors or len(query_vectors) <= k:
                        logging.warning(f"Query vectoe for query {k} is missing")
                        continue
                    ...
                    top_sentence = await self.reranker.rerank(retrieved_chunks=results_sentance_window["documents"][0], query=query)
                    top_fixed = await self.reranker.rerank(retrieved_chunks=results_fixed_size["documents"][0], query=query)
                    ...
                    sentence_context : str = "\n".join(f"Context {i + 1}: {s}" for i, s in enumerate(top_sentence))
                    sentence_prompt: str = f"""You are a Question Answer Assistant who answers the question asked only from the given context.
                    ...
                    The Context: {sentence_context},
                    The Question: {query}"""
                    fixed_context : str = "\n".join(f"Context {i + 1}: {s}" for i, s in enumerate(top_fixed))
                    fixed_prompt: str = f"""You are a Question Answer Assistant who answers the question asked only from the given context.
                    You are not allowed to use any extra knowledge and if there is no answer from the context you should just say honestly that its not found
                    rather than making something up. Answer in complete sentences, not just a number or phrase
                    Do not start your answer with phrases like 'from the given context' or 'based on the context'. Just Answer directly.
                    The Context: {fixed_context},
                    The Question: {query}"""
                    
                    result_sentence = await self.llm.generate(http_client=http_client, prompt=sentence_prompt)
                    result_fixed = await self.llm.generate(http_client=http_client,prompt=fixed_prompt)
                    if result_sentence is None:
                        logging.error("Failed to extract task")
                        raise RuntimeError("Generation returned None for sentence strategy")
                    if result_fixed is None:
                        logging.error("Failed to extract task")
                        raise  RuntimeError("Generation returned None for fixed size strategy")

                    file.write(f"### Query {k + 1}: {query}\n\n")

                    faith_score_sentence = await self.evaluator.get_faithfulness_score(http_client=http_client, retrieved_chunks=results_sentance_window["documents"][0], query=query, result=result_sentence)
                    faith_score_fixed = await self.evaluator.get_faithfulness_score(http_client=http_client, retrieved_chunks=results_fixed_size["documents"][0], query=query, result=result_fixed)


                    relevant_score_sentence = await self.evaluator.get_relevance_score(query=query, result=result_sentence)
                    relevant_score_fixed = await self.evaluator.get_relevance_score(query=query, result=result_fixed)
